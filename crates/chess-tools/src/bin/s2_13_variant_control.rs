...
const CONTROL_MAXIMUM_PLIES: u32 = 4;
const FIXED_NODE_BUDGET: u64 = 8;
const CLOCK_MILLISECONDS: u64 = 10;
const BASELINE_ROLE_IDENTIFIER: u64 = 0x5332_3133_4241_5345;
const CANDIDATE_ROLE_IDENTIFIER: u64 = 0x5332_3133_4341_4e44;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("s2-13 variant control failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.next().is_some() {
        return Err("usage: s2_13_variant_control OUTPUT_DIRECTORY TIER PROTOCOL".into());
    }
    ...
    let source_text = required_environment("S2_13_SOURCE_SHA")?;
    let source_commit = parse_source_commit(&source_text)?;
    let build_identity = required_environment("S2_13_BUILD_IDENTITY")?;
    let exact_invocation = required_environment("S2_13_EXACT_INVOCATION")?;
    ...
    let opening_path = output_directory.join("s2-13-control-openings.tsv");
    ...
    if reparsed != report {
        return Err("serialized S2-13 report did not round-trip exactly".into());
    }
    validate_control_report(&report)?;

    let report_name = format!("s2-13-{tier_text}-{protocol_label}-control.report");
    ...
    write_new(
        &output_directory.join("s2-13-control-manifest.tsv"),
        manifest.as_bytes(),
    )?;

    println!(
        "s2_13_variant_control\ttier={tier_text}\tprotocol={protocol_label}\tpairs={}\tgames={}\tdecision={}\tactivated={}\tchecksum={:016x}",
        plan.pair_count,
        report.games.len(),
        report.decision,
        report.activated(),
...
fn control_plan(value: &str) -> Result<ControlPlan, Box<dyn Error>> {
    match value {
        ...
        _ => Err(format!("unsupported S2-13 validation tier {value:?}").into()),
    }
}
...
fn control_protocol(value: &str) -> Result<EngineVariantResourceProtocol, Box<dyn Error>> {
    match value {
        ...
        _ => Err(format!("unsupported S2-13 resource protocol {value:?}").into()),
    }
}
...
fn parse_source_commit(value: &str) -> Result<[u8; 20], Box<dyn Error>> {
    if value.len() != 40 || !value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err("S2_13_SOURCE_SHA must be exactly 40 hexadecimal characters".into());
    }
    ...
    if result.iter().all(|byte| *byte == 0) {
        return Err("S2_13_SOURCE_SHA must not be all zeroes".into());
    }
    Ok(result)
}
...
fn validate_control_report(report: &EngineVariantValidationReport) -> Result<(), Box<dyn Error>> {
    ...
    if !report.correctness.passed() {
        return Err("S2-13 control correctness pre-gate did not pass".into());
    }
    if report.activated()
    ...
    let expected_games = report
        ...
        .pair_count()
        .checked_mul(2)
        .ok_or("S2-13 control game count overflow")?;
    if report.games.len() != expected_games as usize {
        return Err("S2-13 control report game count does not match paired schedule".into());
    }
    ...
    {
        return Err("S2-13 serialized control report does not match validated structure".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn plans_are_bounded_and_tier_correct() {
        ...
        assert_eq!(production.pair_count, 200);
    ...
    fn protocols_are_explicit_and_bounded() {
        assert_eq!(
            ...
            EngineVariantResourceProtocol::FixedNodes(8)
        );
        assert_eq!(
