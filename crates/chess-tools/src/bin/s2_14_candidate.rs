...
fn main() {
    if let Err(error) = run() {
        eprintln!("S2-14 SEE+LMR candidate evidence failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mode = arguments.next().ok_or(
        "usage: s2_14_candidate deterministic OUTPUT_DIR | clock OUTPUT_DIR | benchmark SAMPLES",
    )?;
    match mode.to_str() {
        Some("deterministic") => {
...
fn run_deterministic(output: &Path) -> Result<(), Box<dyn Error>> {
    ...
    write_new(&output.join("s2-14-parity.tsv"), parity.as_bytes())?;

    let openings_text = control_openings()?;
    write_new(
        &output.join("s2-14-development-openings.tsv"),
    ...
    write_new(
        &output.join("s2-14-deterministic-manifest.tsv"),
        manifest.as_bytes(),
    )?;
    Ok(())
}
...
fn run_clock(output: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let destination = output.join("s2-14-clock-development.report");
    let temporary = output.join(".s2-14-clock-development.tmp");
    write_engine_variant_validation_report_atomic(&destination, &temporary, &report)?;
    let summary = format!(
        "S2_14_CLOCK_SUMMARY\t{REPORT_SCHEMA}\nsource_sha\t{source_text}\nbuild_identity\t{build_identity}\npairs\t{CLOCK_PAIRS}\nclock_milliseconds\t{CLOCK_MILLISECONDS}\ndecision\t{}\nwins\t{}\ndraws\t{}\nlosses\t{}\nunfinished\t{}\nillegal_moves\t{}\ncrashes\t{}\ntime_forfeits\t{}\ninfrastructure_failures\t{}\nmean_bits\t{:016x}\nse_bits\t{:016x}\nlower_bits\t{:016x}\nchecksum\t{:016x}\nactivated\tfalse\n",
        report.decision,
    ...
    write_new(&output.join("s2-14-clock-summary.tsv"), summary.as_bytes())?;
    Ok(())
}
...
fn run_development_match<'a>(
    ...
    let baseline_identity = identity(
        0x5332_3134_4241_5345,
        source_commit,
        build_identity,
        &format!("s2_14_candidate {protocol_name} --role baseline"),
        baseline_policy,
        weights,
    )?;
    let candidate_identity = identity(
        0x5332_3134_4341_4e44,
        source_commit,
        build_identity,
        &format!("s2_14_candidate {protocol_name} --role candidate"),
        candidate_policy,
        weights,
    )?;
...
fn parse_corpus(text: &str) -> Result<Vec<CorpusCase>, Box<dyn Error>> {
    ...
    if lines.next() != Some("CHESS_SEARCH_BASELINE\t1") {
        return Err("invalid S2-14 parity corpus header".into());
    }
    ...
    if cases.is_empty() {
        return Err("S2-14 parity corpus is empty".into());
    }
    Ok(cases)
}
