...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let artifact_path = PathBuf::from(
        args.next()
            .ok_or("usage: s4_candidate_smoke CANDIDATE_ARTIFACT OUTPUT_DIRECTORY PROTOCOL")?,
    );
    let output_directory = PathBuf::from(
        args.next()
            .ok_or("usage: s4_candidate_smoke CANDIDATE_ARTIFACT OUTPUT_DIRECTORY PROTOCOL")?,
    );
    let protocol_text = args
        .next()
        .ok_or("usage: s4_candidate_smoke CANDIDATE_ARTIFACT OUTPUT_DIRECTORY PROTOCOL")?;
    ...
    let expected_value_checksum =
        parse_hex_u64(&required_environment("S4_SMOKE_EXPECTED_VALUE_CHECKSUM")?)?;
    ...
    let candidate =
        EngineVariantRuntime::new(&candidate_identity, &baseline_policy, &candidate_weights)?;
    ...
    fs::write(
        output_directory.join("s4-smoke-openings.tsv"),
        openings_text,
    )?;
