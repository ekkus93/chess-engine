...
fn main() {
    if let Err(error) = run() {
        eprintln!("S2-7 PVS evidence failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mode = arguments
        .next()
        .ok_or("usage: s2_7_pvs deterministic OUTPUT_DIR | clock OUTPUT_DIR | benchmark SAMPLES")?;
    match mode.to_str() {
        Some("deterministic") => {
        ...
        _ => Err(
            "usage: s2_7_pvs deterministic OUTPUT_DIR | clock OUTPUT_DIR | benchmark SAMPLES"
                .into(),
        ),
    }
}
...
fn run_deterministic(output: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let source_text = required_environment("S2_7_SOURCE_SHA")?;
    let source_commit = parse_source_commit(&source_text)?;
    let build_identity = required_environment("S2_7_BUILD_IDENTITY")?;
    ...
    weights.validate()?;
    if baseline_policy.policy.principal_variation_search_enabled()
        || !candidate_policy.policy.principal_variation_search_enabled()
    {
        return Err("S2-7 policy activation boundary is inverted".into());
    }

    let parity = run_parity_corpus(&baseline_policy, &candidate_policy, &weights)?;
    write_new(&output.join("s2-7-parity.tsv"), parity.as_bytes())?;

    let openings_text = control_openings()?;
    write_new(
        &output.join("s2-7-development-openings.tsv"),
    ...
    write_new(
        &output.join("s2-7-deterministic-manifest.tsv"),
        manifest.as_bytes(),
    )?;
    Ok(())
}
...
fn run_clock(output: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let destination = output.join("s2-7-clock-development.report");
    let temporary = output.join(".s2-7-clock-development.tmp");
    write_engine_variant_validation_report_atomic(&destination, &temporary, &report)?;
    let summary = format!(
        "S2_7_CLOCK_SUMMARY\t{REPORT_SCHEMA}\nsource_sha\t{source_text}\nbuild_identity\t{build_identity}\npairs\t{CLOCK_PAIRS}\nclock_milliseconds\t{CLOCK_MILLISECONDS}\ndecision\t{}\nwins\t{}\ndraws\t{}\nlosses\t{}\nunfinished\t{}\nillegal_moves\t{}\ncrashes\t{}\ntime_forfeits\t{}\ninfrastructure_failures\t{}\nmean_bits\t{:016x}\nse_bits\t{:016x}\nlower_bits\t{:016x}\nchecksum\t{:016x}\nactivated\tfalse\n",
        report.decision,
    ...
    write_new(&output.join("s2-7-clock-summary.tsv"), summary.as_bytes())?;
    Ok(())
}
...
fn run_development_match<'a>(
    ...
    let baseline_identity = identity(
        0x5332_3742_4153_4531,
        source_commit,
        build_identity,
        &format!("s2_7_pvs {protocol_name} --role baseline"),
        baseline_policy,
        weights,
    )?;
    let candidate_identity = identity(
        0x5332_3743_414e_4431,
        source_commit,
        build_identity,
        &format!("s2_7_pvs {protocol_name} --role candidate"),
        candidate_policy,
        weights,
    )?;
...
fn run_benchmark(samples: usize) -> Result<(), Box<dyn Error>> {
    ...
    if baseline.aggregate.pvs_zero_window_searches != 0 || baseline.aggregate.pvs_researches != 0 {
        return Err("baseline benchmark unexpectedly exercised PVS".into());
    }
    if candidate.aggregate.pvs_zero_window_searches == 0
        || candidate.aggregate.pvs_researches > candidate.aggregate.pvs_zero_window_searches
    {
...
fn parse_corpus(text: &str) -> Result<Vec<CorpusCase>, Box<dyn Error>> {
    ...
    if lines.next() != Some("CHESS_SEARCH_BASELINE\t1") {
        return Err("invalid S2-7 parity corpus header".into());
    }
    ...
    if cases.is_empty() {
        return Err("S2-7 parity corpus is empty".into());
    }
    Ok(cases)
}
...
fn parse_source_commit(value: &str) -> Result<[u8; 20], Box<dyn Error>> {
    if value.len() != 40 || !value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err("S2_7_SOURCE_SHA must be exactly 40 hexadecimal characters".into());
    }
    ...
    if result.iter().all(|byte| *byte == 0) {
        return Err("S2_7_SOURCE_SHA must not be all zeroes".into());
    }
    Ok(result)
}
