...
fn kings_only_builder() -> PositionBuilder {
    ...
    builder
        .place_piece(square("e1"), Piece::new(Color::White, PieceKind::King))
        .expect("white king square is empty");
    builder
        .place_piece(square("e8"), Piece::new(Color::Black, PieceKind::King))
...
fn playable_builder_requires_exactly_one_king_per_color() {
    ...
    multiple
        .place_piece(square("d1"), Piece::new(Color::White, PieceKind::King))
...
fn invalid_en_passant_metadata_is_rejected() {
    ...
    let mut occupied = kings_only_builder()
        ...
        .with_en_passant(Some(square("e3")));
    occupied
        .place_piece(square("e3"), Piece::new(Color::White, PieceKind::Pawn))
...
fn internal_editor_updates_all_redundant_structures() {
    let mut position = kings_only_builder()
        .build_playable()
        .expect("kings are valid");
...
fn failed_internal_edits_do_not_change_logical_state() {
    ...
    assert!(position.editor().remove_piece(square("e1")).is_err());
