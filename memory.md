...
## 2026-11-20T02:11:30Z - GPT-4.5-alpha - Alpha-beta pruning integration complete
...
## 2026-55-20T02:55:46Z - GPT-4.5-alpha - Linting fixes complete
...
### Issue Summary
Alpha-beta pruning is implemented but is NOT pruning:
- Alpha/beta window is too wide.
- Alpha/beta values do not update correctly across recursive calls.
- Node counts are identical with/without pruning; execution time is nearly identical.
- Alpha-beta cutoffs never fire in normal positions; the search behaves like vanilla minimax.

### Files affected
- chess_game/chess/ai.py
  - minimax(board, params)
  - _search_move_loop(board, legal_moves, scored_moves, params)
  - MinimaxParams.alpha, MinimaxParams.beta
  - Iterative deepening window logic in get_best_move(board, depth)

### What we tried (and failed)
- Widened vs tightened alpha/beta:
  - Tested: alpha=-1_000_000_000 beta=1_000_000_000 vs alpha=-50 beta=50 vs alpha=-100 beta=100.
  - Result: 0.0% pruning savings; node counts identical.
- Debug prints:
  - Patched _search_move_loop with prints at prune conditions.
  - Confirmed pruning conditions exist but alpha/beta do not propagate correctly.
- Updated alpha/beta in alpha/beta window:
  - Adjusted iterative deepening window: score ± 1000 → ± 50.
  - Still no effective pruning.
- Checked logic:
  - Alpha-beta cutoffs are implemented.
  - TT and TSCP-style lookup are integrated.
  - Alpha/beta values do not update across recursive calls.

### Expected behavior
- Alpha-beta pruning should reduce nodes searched (at least 10-30% at depth 3-5).
- Iterative deepening should use a tighter window.
- Alpha/beta should update across recursive calls.

### For ChatGPT 5.5
- Review ai.py minimax implementation for alpha-beta pruning correctness.
- Ensure alpha/beta update across recursive calls.
- Ensure iterative deepening uses a tighter window.
- Ensure pruning works at depth 3-5.
