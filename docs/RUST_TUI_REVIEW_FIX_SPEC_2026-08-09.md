...
- RF-006.4 decision: added overwrite confirmation (new `ConfirmationAction::OverwriteSave`, reusing the existing `Overlay::Confirmation` mechanism) but explicitly did not add path-traversal sanitization — this is a local single-user "type your own destination" flow, and the overwrite confirmation is the guardrail that matters (an accidental/typo destination now requires an explicit "yes, overwrite" instead of silently succeeding). Path sanitization remains a possible future hardening item, not done here.
- RF-006.1 sub-decision: introduced `GameSession::clear_search()` as a structural fix (5 call sites now delegate to it) rather than a full cross-struct `SearchSlot` type unifying `AppState.pending_search` with `GameSession.{active_search,thinking}` — judged too invasive since those fields live on two different structs; deferred.
