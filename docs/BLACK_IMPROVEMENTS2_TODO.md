...
- [ ] Verify that `tmp/black_improvements1_game1.txt`, `game2.txt`, and `game3.txt`
      show Black never castling.
- [ ] Check move 8 in each game: confirm Black plays Na5 with castling rights intact.
- [ ] Check move 10: confirm g7-g5 is played with f8 bishop still on f8 (blocking
      kingside castling).
- [ ] Confirm `opening_king_urgency_penalty` and `opening_king_safety_score` are
      already wired into evaluation — audit their scale vs. the tactical gain from Na5.

### 0.2 Define success criteria

- [ ] In a fresh depth-3 self-play game, Black castles at least once per game.
- [ ] g7-g5 before castling (with f8 bishop blocking) is suppressed at depth=3.
- [ ] Black considers queenside castling when the kingside is compromised.
- [ ] Na5 with castling rights still held scores lower than a developing/castling move.
...
- [ ] Construct a board from `black_improvements1_game1.txt` at move 13 (Black to
      move, king at e8, castling rights intact, g5 pawn advanced).
- [ ] Assert `get_best_move(board, depth=3)` chooses a castling or castling-enabling
      move rather than another wing pawn push or queen move.
- [ ] Add an ordering test: `_move_order_score(board, e8g8, None)` >
...
- [ ] Read `opening_development.py` — find `opening_king_safety_score` and
      `opening_king_urgency_penalty`.  Check their current magnitude and phase gate.
- [ ] Read `opening_move_ordering.py` — find `QUIET_OPENING_CASTLING_URGENCY_BONUS`.
      Confirm it fires for e8-g8 and e8-c8.
- [ ] Check whether the bonus scales with move count (should be larger the longer
      the king has been uncastled with queens on the board).

### 1.3 Strengthen late-opening castling urgency

- [ ] In `opening_development.py`, add a `late_castling_urgency_penalty` that scales
...
- [ ] Gate the penalty to positions where:
...
- [ ] In `ai_search_helpers.py` (`_strategic_root_bonus`), add a root-only bonus
      for castling moves when the king has been uncastled for more than 10 moves.
- [ ] The bonus should be large enough (≥ 40 cp) to override near-equal alternatives.
...
- [ ] Construct the board at move 9 from the validation games (Black to move, king
      at e8, f8 bishop at home, kingside castling rights intact).
- [ ] Assert `quiet_strategy_order_score(board, g7g5, None)` <
...
- [ ] In `opening_move_ordering.py`, extend `_is_castled_shelter_pawn_advance` (or
...
- [ ] The additional penalty for this "castling-blocking" condition should be larger
      than the plain shelter advance penalty (≥ 40 cp total ordering penalty).

### 2.3 Add evaluation signal

- [ ] In `pawn_structure_evaluation.py` or `opening_development.py`, add a penalty
...
- [ ] Wire the new term into `evaluation.py` under `development`.
...
- [ ] Reconstruct the move-8 board from the validation games.
- [ ] Assert `quiet_strategy_order_score(board, c6a5, None)` <
      `quiet_strategy_order_score(board, e8g8, None)` (castling is always preferred).
- [ ] Assert the gap between castling and Na5 in ordering is ≥ 60 cp.

### 4.2 Add castling-delay cost to rim-knight penalty

- [ ] In `opening_move_ordering.py`, extend `_is_knight_wing_drift` or add a
...
- [ ] In `opening_development.py`, extend `middlegame_rim_knight_penalty` to scale
...
For each game check:
- [ ] Does Black castle (e8-g8 or e8-c8) before move 20?
- [ ] Does Black push g7-g5 before castling?
- [ ] Does Black's king survive into the endgame more often?
- [ ] What is the game length and result?

Record findings in `tmp/black_improvements2_validation.txt`.

### 5.3 Accept or iterate

- [ ] If Black castles in ≥ 2 of 3 games → mark this TODO complete.
- [ ] If castling still doesn't happen → revisit Task 1 and increase the urgency
      penalty, then re-run.
...
- [ ] Update `docs/BLACK_IMPROVEMENTS2_TODO.md` to mark all tasks complete.
