...
- [ ] **0a** Record the three key failure positions from `tmp/strategy10_acceptance.txt`:
...
- [ ] **0b** Note the opening: White's move 1 is still Nc3. Save notes to `tmp/strategy11_baseline_notes.txt`.
- [ ] **0c** Create `tests/test_ai_strategy11_regressions.py` with board helpers for:
...
- [ ] **0d** Add baseline assertions that expose the current weaker behaviors.
...
- [ ] **1a** Audit `_opening_central_pawn_root_bonus()` to understand why it does not fire on move 1 even though the opening gate is active.
- [ ] **1b** Add a `_move1_central_pawn_bonus()` root helper that fires specifically when:
...
- [ ] **1c** Call `_move1_central_pawn_bonus()` from `_opening_root_bonus()` and ensure it supersedes the existing move-1 quiet ordering.
- [ ] **1d** Confirm the bonus does not apply after move 1 (guard on ply count or piece density on starting squares).
- [ ] **1e** Add regression test `test_strategy11_white_prefers_e4_or_d4_on_move1`.
- [ ] **1f** Add regression test `test_strategy11_move1_bonus_does_not_fire_in_late_opening`.
...
- [ ] **2a** Write a `_piece_is_immediately_capturable(board, square, color)` helper in `ai_move_ordering.py` (or a new `move_safety_guidance.py`) that returns True if any opponent piece attacks `square` and the capture would be material-positive for the opponent.
- [ ] **2b** Add a `_advantage_preservation_quiet_penalty(board, move, side, material_lead)` function:
...
- [ ] **2c** Feed the new penalty into `_get_quiet_move_score()` in `ai_move_ordering.py`.
- [ ] **2d** Ensure the penalty does not fire in equal or behind positions, nor during captures.
- [ ] **2e** Add regression test `test_strategy11_black_avoids_hanging_pieces_when_ahead`.
- [ ] **2f** Add regression test `test_strategy11_safety_penalty_inactive_in_equal_positions`.
...
- [ ] **3a** Add a `_anti_queen_trade_root_penalty(board, move, side, context)` helper in `conversion_guidance.py`:
...
- [ ] **3b** Call the penalty from `winning_conversion_root_bonus()` when the context is active.
- [ ] **3c** Tune `_ANTI_QUEEN_TRADE_PENALTY` so it overrides typical positional tie-breaks but not clearly winning tactical moves.
- [ ] **3d** Add regression test `test_strategy11_black_avoids_queen_trade_when_clearly_ahead`.
- [ ] **3e** Add regression test `test_strategy11_anti_queen_trade_inactive_in_level_positions`.
...
- [ ] **4a** Audit `simple_endgame_guidance.py` to understand where pure R vs B+K is handled (or missing).
- [ ] **4b** Add a `_rook_vs_bishop_king_conversion_bonus(board, side)` helper in `endgame_evaluation.py`:
...
- [ ] **4c** Feed the helper into `endgame_evaluation()` score.
- [ ] **4d** Ensure the helper does not fire in double-rook or queen endgames.
- [ ] **4e** Add regression test `test_strategy11_rook_vs_bishop_king_activates_king`.
- [ ] **4f** Add regression test `test_strategy11_rook_vs_bishop_king_cuts_off_king_with_rook`.
...
- [ ] **5a** Audit the current rook endgame handling to identify what is already scored and what is missing for R+B vs R.
- [ ] **5b** Add a `_rook_bishop_vs_rook_conversion_bonus(board, side)` helper:
...
- [ ] **5c** Feed the helper into `endgame_evaluation()`.
- [ ] **5d** Ensure the helper stays quiet in balanced R+B vs R positions (no pawn imbalances, no king exposure).
- [ ] **5e** Add regression test `test_strategy11_rook_bishop_vs_rook_coordinates_pieces`.
- [ ] **5f** Add regression test `test_strategy11_rook_bishop_vs_rook_inactive_in_equal_rook_endgame`.
...
- [ ] **6a** Run `python -m ruff check chess_game tests` and fix every warning.
- [ ] **6b** Run `python -m mypy chess_game/` and fix every type issue.
- [ ] **6c** Run `python -m pylint chess_game/` and keep the score at `10.00/10`.
- [ ] **6d** Run `python -m pytest tests/ -q` and fix any failing regression.
- [ ] **6e** Run a new depth-3 vs depth-3 self-play game and save the transcript under `tmp/strategy11_acceptance.txt`.
- [ ] **6f** Check whether:
...
- [ ] **6g** Write a short review note under `tmp/strategy11_review.txt` summarising move count, White move-1 quality, advantage preservation, and endgame technique.
...
- [ ] **7b** Commit with a clear message describing the move-1 bonus, advantage-preservation penalties, and endgame-technique helpers.
- [ ] **7c** Push to `origin/master`.
- [ ] **7d** Update all checklist items above to reflect final completion.
