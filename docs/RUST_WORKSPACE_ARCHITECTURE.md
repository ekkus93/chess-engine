...
|---|---|---|---|
| `chess-core` | library | Position model, moves, attacks, rules, FEN/UCI/SAN notation, game history, hashing, and exact perft | none |
...
`chess-app` is not an engine layer. It is an application/session layer for interactive human-facing frontends. Rules and SAN notation remain authoritative in `chess-core`; evaluation/search/cancellation remain authoritative in `chess-search`.
...
- presentation-only Moves/Engine tab and internal move-list scroll state;
- grouping Rust-generated SAN history into numbered display rows;
- promotion, restart, resign, new-game, error, and cleanup-retry presentation;
...
The Android application does **not** own a second chess rule engine, SAN formatter, opening-book policy, or independent interactive game controller. `ChessGame` snapshots provide authoritative FEN, legal UCI moves, UCI move history, Rust-generated SAN history, turn state, outcome, and search metrics from the Rust `GameController`. Android derives selectable squares from the Rust legal-move list and submits the selected UCI move back to Rust.

## SAN ownership

SAN is chess notation logic and therefore lives in `chess-core`, without Android/JNI dependencies. Formatting uses authoritative pre/post move state rather than attempting to reconstruct notation from UCI text in a presentation layer. SAN formatting must not mutate the authoritative game state.

The high-level Android snapshot protocol carries both histories:

- UCI remains the exact move-identity/control representation used for legal-move projection, submission, last-move highlighting, and internal/test behavior;
- SAN is the player-facing notation generated by Rust and rendered by Android.

A wire-shape change requires an explicit snapshot protocol version change plus Rust encoder, Kotlin parser, and host-JVM contract updates.
...
Opaque Android game handles remain registered until explicit native close completes successfully. Kotlin clears its corresponding handle only after native destruction succeeds, keeping cleanup failures visible and retryable. Once `ChessGame.create()` succeeds, the ViewModel retains that owner before requesting the first snapshot. If initial snapshot acquisition and its immediate explicit close both fail, the reachable owner remains retained, new-game configuration is blocked, both failures remain visible, and the user receives an explicit cleanup-retry action. A phantom-reference reaper is a last-resort leak backstop after an owner is already unreachable, not a substitute for explicit close.
...
- Opening-book errors remain failures rather than silently falling through to search.
