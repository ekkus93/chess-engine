...
- [x] Replay `tmp/white_improvements1_game2.txt` to the position where White
      castled (move 47) and confirm the king was still on e1 through move 46.
- [x] Replay `tmp/white_improvements1_game1.txt` to the position just after
      White castled (move 19) and confirm h2-h3 was not played until move 103.
- [x] Replay `tmp/white_improvements1_game3.txt` to move 88 (Black plays d4
      toward promotion) and record the board state.
- [x] For each position, run `get_best_move(board, depth=3)` and identify
      what the engine actually plays vs. the correct defensive choice.
- [x] Save baseline board states in `tmp/white_improvements2_baseline.txt`.

### 0.2 Audit existing signals

- [x] Audit `late_castling_urgency_penalty` from BLACK_IMPROVEMENTS2 — does
      it fire for White in game 2's position at fullmove 10-20?  Is the
      threshold (fullmove > 4) too lenient?
- [x] Audit `h_pawn_exposure_penalty` from WHITE_IMPROVEMENTS1 — why is h3
      deferred to move 103/117?  Is the penalty large enough to override
      middlegame plans at depth=3?
- [x] Check `_h_pawn_luft_root_bonus` in `ai_search_helpers.py` — does the
      +36 root bonus fire even when h2 is NOT yet threatened (to encourage
      prophylactic h3)?
- [x] Audit endgame evaluation for queen-vs-rook and pawn promotion races.

### 0.3 Define success criteria

- [x] White plays h2-h3 within 5 moves of castling in ≥ 2 of 3 new games.
- [x] White castles by move 25 in all 3 new games.
- [x] White does not lose queen-vs-rook endgames due to passive play.
- [x] White correctly stops opponent pawn promotion when ahead in material.
...
- [x] Construct the game-1 board at move 20 (just after White castled on
      move 19, no bishop threat yet).
- [x] Assert `quiet_strategy_order_score(board, h2h3, None)` is in the
      top-5 quiet moves even with no visible bishop threat.
- [x] Assert the ordering gap between h2-h3 and the next best quiet pawn
      move is ≥ 20 cp when the king has just castled kingside with queens
      on the board.

### 1.2 Add prophylactic post-castling luft bonus

- [x] In `opening_move_ordering.py` or `ai_move_ordering.py`, add a
...
- [x] The bonus should be ≥ 24 cp so it consistently beats passive
      piece shuffles in the first few moves after castling.
- [x] This is a *prophylactic* bonus — it fires even when no piece
      currently threatens h2.

### 1.3 Wire into root tie-break

- [x] In `ai_search_helpers.py`, extend `_h_pawn_luft_root_bonus` to
      also fire prophylactically (not only when `h_pawn_exposure_penalty
      >= 15`) using the same just-castled condition from 1.2.
- [x] The root bonus should be ≥ 30 cp in the prophylactic case.
...
- [x] Construct the game-2 board at fullmove 15 (around move 29-30, king
...
- [x] Assert `quiet_strategy_order_score(board, e1g1, None)` >
      `quiet_strategy_order_score(board, Re1e2, None)` + 40 at this position.

### 2.2 Increase late-castling urgency for later moves

- [x] In `opening_development.py`, increase `_LATE_CASTLING_BASE_PENALTY`
      from 12 to 16 so the scaling penalty grows faster after fullmove 4.
- [x] Add a hard cap increase: `_LATE_CASTLING_MAX_PENALTY` from 96 to 128.
- [x] Consider adding a secondary "mid-game uncastled" penalty that fires
...
- [x] Verify the increased bonus does not cause regressions in positions
...
- [x] Construct a KQvKR ending (Black queen, White rook).
- [x] Assert that the engine recognizes the king must centralise and the
      rook must stay active (avoid passive rank-1 shuffling).
- [x] Assert `get_best_move(board, depth=3)` prefers rook centralisation
      over passive back-rank moves in the relevant endgame position.

### 3.2 Audit existing queen-vs-rook endgame guidance

- [x] Check `endgame_evaluation.py` — is there any KQvKR-specific logic?
- [x] Check `rook_endgame_guidance.py` — does the passive rook penalty
      (`_WORSE_SIDE_CHECK_DRIFT_PENALTY`) apply to the losing side in a
      queen-vs-rook ending?
- [x] Check `defensive_endgame_guidance.py` — does it recognize the need
      for king activity in this ending type?

### 3.3 Add queen-vs-rook endgame signals

- [x] In `endgame_evaluation.py`, add a `evaluate_queen_vs_rook(board,
...
- [x] Wire the new function into the evaluation breakdown.
- [x] In `ai_move_ordering.py`, add a quiet-order penalty for rook moves
...
- [x] Construct the game-3 board near move 88 (Black's d4 pawn about
      to promote).
- [x] Assert `get_best_move(board, depth=3)` prioritises stopping the
      d4 promotion over continuing White's own plans.
- [x] Assert that `passer_race_guidance` rates Black's d4 pawn as a
      critical threat and raises the evaluation accordingly.

### 4.2 Audit passer race detection

- [x] Check `passer_race_guidance.py` — does `_is_relevant_passer_race`
      fire correctly when both sides have near-promotion passers?
- [x] Check `_is_near_promotion_passer_push` — does it detect Black's d4
      pawn push (one or two squares from queening) as a high-priority threat?
- [x] Check `low_material_race_guidance.py` — does `endgame_race_context`
      correctly classify the position as a "must-hold" race for White?

### 4.3 Strengthen opposing-passer urgency

- [x] In `passer_race_guidance.py`, ensure `_ENEMY_PASSER_DANGER_BONUS`
      is large enough that a near-promotion enemy passer in a race outweighs
      White's own promotion plans.
- [x] In `ai_search_helpers.py`, add a root-only penalty for ignoring a
      ...
      address it.
- [x] Gate the penalty to low-material positions (≤ 6 non-pawn pieces).
...
For each game, check:
- [x] Does White castle by move 25?
- [x] Does White play h2-h3 within 5 moves of castling?
- [x] Does White avoid losing KQvKR endgames through passive play?
- [x] Does White stop near-promotion enemy pawns in endgame races?
- [x] What are game lengths and results?

Record findings in `tmp/white_improvements2_validation.txt`.

### 5.3 Accept or iterate

- [x] If White castles by move 25 in all 3 games → Task 2 success.
- [x] If h3 appears within 5 moves of castling in ≥ 2 games → Task 1 success.
- [x] If no KQvKR losses through passive rook play → Task 3 success.
- [x] If no pawn-race blunders → Task 4 success.
- [x] If any criterion fails → revisit the relevant task and increase the
      signal, then re-run.
...
- [x] Update `docs/WHITE_IMPROVEMENTS2_TODO.md` to mark all tasks complete.
