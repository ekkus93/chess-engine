...
- [ ] Replay `tmp/selfplay_varied_game2.txt` up to move 15 (White's Nxe5 blunder)
      and record the exact board state.
- [ ] Replay to move 19 (White castles, h2 undefended) and record board state.
- [ ] Replay to move 25 (last move before Bxh2+) and record board state.
- [ ] For each position, run `get_best_move(board, depth=3)` and confirm the
      bad move is chosen.
- [ ] Save all three board states in `tmp/white_improvements1_baseline.txt`.

### 0.2 Audit existing signals

- [ ] Check `opening_development.py` — does `opening_king_safety_score` or
      `_uncastled_shell_penalty` detect an undefended h2/h7 after castling?
- [ ] Check `pawn_structure_evaluation.py` — does `_shelter_file_gap_penalty`
      fire for a missing h2/g2 pawn when the king is castled?
- [ ] Check `ai_move_ordering.py` — does `QUIET_URGENT_LUFT_BONUS` or any
      luft signal fire at move 19-25 of game 2?
- [ ] Check `defensive_priorities.py` — does `king_danger_index` or
      `king_defense_profile` flag the h2 weakness?
- [ ] Document each gap in `tmp/white_improvements1_baseline.txt`.

### 0.3 Define success criteria

- [ ] At depth=3, White plays h3 (or g3) within the first 5 moves after
      castling when h2 is undefended and queens are on the board.
- [ ] At depth=3, White does not play Nxe5 in the game-2 position because
      the fork is answered by a pawn recapture.
- [ ] A fresh self-play game no longer ends in the Bxh2+ mating pattern
      within 35 moves.
...
- [ ] Create (or extend) `tests/test_ai_white_improvements1.py`.
- [ ] Reconstruct the move-19 board from game 2 (White just castled, h2 bare).
- [ ] Assert `quiet_strategy_order_score(board, h2h3, None)` is among the top-3
      scoring quiet moves — i.e., the ordering strongly encourages luft.
- [ ] Assert `get_best_move(board, depth=3)` does **not** choose a purely
      passive move (Re2, Bd2) when h2 is undefended and a bishop is aimed at it.
- [ ] Add a move-ordering test for the castled position:
      `_move_order_score(board, h2h3, None)` > `_move_order_score(board, Re1e2, None)`.

### 1.2 Audit existing luft signals

- [ ] Find `QUIET_LUFT_BONUS` and `QUIET_URGENT_LUFT_BONUS` in `ai_move_ordering.py`.
      Record their current values and the conditions that fire them.
- [ ] Check `_defensive_priority_bonus` in `ai_move_ordering.py` — does it
      reward luft creation or only active defense?
- [ ] Check `king_needs_shelter` in `defensive_priorities.py` — does it detect
      the h2-undefended pattern?

### 1.3 Add "h2/h7 exposed" evaluation penalty

- [ ] In `opening_development.py` or `pawn_structure_evaluation.py`, add a
...
- [ ] Make the penalty proportional to how directly the attacking piece threatens
      h2/h7 (a bishop one move away = maximum penalty; two moves away = half).
- [ ] Wire the new penalty into `evaluation.py` under the `king_safety` or
      `king_exposure` breakdown key.

### 1.4 Strengthen the luft ordering bonus

- [ ] In `ai_move_ordering.py`, increase `QUIET_LUFT_BONUS` or add a new
...
- [ ] The bonus should be large enough (≥ 36 cp) that h3 beats passive rook
      shuffles in ordering.

### 1.5 Add root tie-break signal

- [ ] In `ai_search_helpers.py` (`_strategic_root_bonus`), add a root-only
      bonus for h-pawn luft moves when the king is castled and the h-file
      shelter is gone or threatened.
- [ ] Gate the bonus to positions where the enemy has a piece that could
...
- [ ] Reconstruct the move-25 board (position just before Bxh2+).
- [ ] Assert `king_danger_index(board, Color.WHITE)` ≥ some threshold that
      would trigger defensive priority ordering.
- [ ] Assert that `quiet_strategy_order_score` rewards defensive moves
      (g2-g3, Rf1-h1) over offensive moves (Re2-e3) in this position.
- [ ] Assert that the move-order score for h2-h3 or g2-g3 is highest among
      all quiet moves in this position.

### 2.2 Add diagonal bishop-threat detection to king danger

- [ ] In `defensive_priorities.py`, extend `king_defense_profile` or
...
- [ ] A bishop on d6 targeting h2 with no pieces in between should raise the
      danger score by at least 2 points (equivalent to an open invasion line).
- [ ] A bishop on c5/e3 with one piece in between should raise it by 1 point.

### 2.3 Wire the new danger signal into ordering

- [ ] Ensure the higher danger score from 2.2 causes `_defensive_priority_bonus`
      in `ai_move_ordering.py` to rank defensive moves (h3, g3, Rh1) higher.
- [ ] Confirm that `QUIET_DANGER_RELIEF_BONUS` fires for moves that block the
...
- [ ] Reconstruct the move-14 board (White to move after Black played ...e7-e5).
- [ ] Assert `get_best_move(board, depth=3)` does **not** choose Nf3-e5.
- [ ] Assert `capture_order_score(board, Nf3e5)` is negative or very low when
      the landing square can be immediately recaptured by an enemy pawn.
- [ ] Assert that a defensive move (e.g. Bf4-e3 or d4xe5) scores higher than
      Nf3-e5 in quiet or capture ordering at this position.

### 3.2 Add pseudo-fork detection to capture ordering

- [ ] In `ai_capture_ordering.py`, add a penalty for captures where:
...
- [ ] Name the function `_pawn_recapture_risk_penalty` or similar.
- [ ] The penalty should be large enough (≥ piece value difference) to make
      such captures rank below safe alternatives in ordering.

### 3.3 Extend evaluation for hanging piece after fork attempt

- [ ] In `evaluation.py` or `piece_coordination.py`, add a small evaluation
...
- [ ] This is a static signal: "if any own piece is attacked by an enemy pawn
      and not defended, penalise the position."
- [ ] Gate the penalty to middlegame positions to avoid over-penalising
      endgame pawn chains.
...
- [ ] Reconstruct the move-16 board (after Black dxe5, White to move).
- [ ] Assert that `quiet_strategy_order_score(board, Bf4d2, None)` is **lower**
      than an active bishop move (e.g. Bf4-e3 developing/defending).
- [ ] Assert that the bishop-retreat-to-back-rank penalty from BLACK_IMPROVEMENTS1
      also fires for White's bishops (it should — the penalty is symmetric).

### 4.2 Audit the existing bishop penalty

- [ ] Check `_bishop_passive_retreat_penalty` in `ai_move_ordering.py` — does
      it fire for White's Bf4-d2 move?  (d2 is NOT the back rank for White, so
      it may not fire.)
- [ ] If Bf4-d2 is not caught, extend the penalty to cover moves that:
...
- [ ] In `ai_move_ordering.py`, add or extend `_bishop_passive_retreat_penalty`
...
- [ ] The penalty should be proportional to mobility loss.
...
- [ ] Update `docs/WHITE_IMPROVEMENTS1_TODO.md` to mark all tasks complete.
