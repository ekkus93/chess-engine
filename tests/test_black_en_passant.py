...
from chess_game.test_util import create_piece, clear_board, _setup_kings
...
def test_black_en_passant_legal_example() -> None:
    ...
    # White captures en passant from d2 to e7 (the en_passant target)
    board.turn = Color.WHITE
    assert (
        board.make_move(
            ConstantSquare(row=ROW_2, col=COL_D), ConstantSquare(row=ROW_7, col=COL_E)
        )
        is True
    )
    assert board.get_piece(ConstantSquare(row=ROW_2, col=COL_D)) is None
    assert (
        board.get_piece_type_at(ConstantSquare(row=ROW_7, col=COL_E)) == PieceType.PAWN
    )
    assert board.get_color_at(ConstantSquare(row=ROW_7, col=COL_E)) == Color.WHITE
