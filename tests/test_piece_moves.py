...
def test_rook_valid_vertical_move() -> None:
    ...
    assert (
        board.is_valid_rook_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_1, col=COL_E)
        )
        is True
    )
    assert (
...
def test_rook_blocked_by_friendly_piece_in_path() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_5, col=COL_C), create_piece(Color.WHITE, PieceType.PAWN)
    )
    assert (
        board.is_valid_rook_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_A)
        )
        is False
    )
    board.set_piece(
        ConstantSquare(row=get_row_constant(4), col=get_col_constant(2)),
...
def test_rook_blocked_by_enemy_piece_before_destination() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_5, col=COL_C), create_piece(Color.BLACK, PieceType.PAWN)
    )
    assert (
        board.is_valid_rook_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_A)
...
def test_rook_cannot_move_diagonally() -> None:
    ...
    assert (
        board.is_valid_rook_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_3, col=COL_C)
        )
        is False
    )
    assert (
        board.is_valid_rook_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(2), col=get_col_constant(2)),
        )
        is False
    )
...
def test_bishop_valid_diagonal_move() -> None:
    ...
    assert (
        board.is_valid_bishop_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_2, col=COL_B)
        )
        is True
    )
    assert (
        board.is_valid_bishop_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(1), col=get_col_constant(1)),
        )
        is True
    )
...
def test_bishop_cannot_move_straight() -> None:
    ...
    assert (
        board.is_valid_bishop_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_B)
        )
        is False
    )
    assert (
        board.is_valid_bishop_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(1)),
        )
        is False
    )
...
def test_bishop_cannot_capture_friendly_piece() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_2, col=COL_B),
...
def test_queen_illegal_knight_like_move() -> None:
    ...
    assert (
        board.is_valid_queen_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_3, col=COL_D)
        )
        is False
    )
    assert (
        board.is_valid_queen_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(2), col=get_col_constant(3)),
        )
        is False
    )
...
def test_knight_valid_l_move_both_orientations() -> None:
    ...
    assert (
        board.is_valid_knight_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_4, col=COL_C)
        )
        is True
    )
    assert (
    ...
    assert (
        board.is_valid_knight_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(3), col=get_col_constant(2)),
        )
        is True
    )
...
def test_knight_can_jump_over_pieces() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_5, col=COL_D), create_piece(Color.WHITE, PieceType.PAWN)
    )
    assert (
        board.is_valid_knight_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_3, col=COL_D)
    ...
    board.set_piece(
        ConstantSquare(row=get_row_constant(3), col=get_col_constant(4)),
        create_piece(Color.WHITE, PieceType.PAWN),
    )
    board.set_piece(
        ConstantSquare(row=get_row_constant(4), col=get_col_constant(3)),
        create_piece(Color.WHITE, PieceType.PAWN),
    )
    assert (
        board.is_valid_knight_move(
...
def test_knight_illegal_straight_move() -> None:
    ...
    assert (
        board.is_valid_knight_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_C)
        )
        is False
    )
    assert (
...
def test_knight_cannot_capture_friendly_piece() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_3, col=COL_D), create_piece(Color.WHITE, PieceType.PAWN)
    )
    assert (
        board.is_valid_knight_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_3, col=COL_D)
...
def test_king_one_step_orthogonal() -> None:
    ...
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_F)
        )
        is True
    )
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(5)),
        )
        is True
    )
...
def test_king_one_step_diagonal() -> None:
    ...
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_6, col=COL_F)
        )
        is True
    )
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(5), col=get_col_constant(5)),
        )
        is True
    )
...
def test_king_cannot_move_two_squares() -> None:
    ...
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=ROW_5, col=COL_E), ConstantSquare(row=ROW_5, col=COL_G)
        )
        is False
    )
    assert (
        board.is_valid_king_move(
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(6)),
        )
        is False
    )
...
def test_white_pawn_one_step_from_e2_to_e3() -> None:
    ...
    assert (
        board.is_valid_pawn_move(
            ConstantSquare(row=get_row_constant(1), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(2), col=get_col_constant(4)),
        )
        is True
    )
...
def test_white_pawn_blocked_on_two_step_intermediate_square() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=get_row_constant(5), col=get_col_constant(4)),
        create_piece(Color.BLACK, PieceType.KNIGHT),
    )
    assert (
        board.is_valid_pawn_move(
            ConstantSquare(row=get_row_constant(6), col=get_col_constant(4)),
            ConstantSquare(row=get_row_constant(4), col=get_col_constant(4)),
        )
        is False
    )
...
def test_white_pawn_capture_diagonally() -> None:
    ...
    assert (
        board.is_valid_pawn_move(
            ConstantSquare(row=ROW_2, col=COL_E), ConstantSquare(row=ROW_3, col=COL_F)
        )
        is True
    )
    board.set_piece(
...
def test_black_pawn_diagonal_capture() -> None:
    ...
    board.set_piece(
        ConstantSquare(row=ROW_6, col=COL_D),
        create_piece(Color.WHITE, PieceType.KNIGHT),
    )
    assert (
        board.is_valid_pawn_move(
            ConstantSquare(row=ROW_7, col=COL_E), ConstantSquare(row=ROW_6, col=COL_D)
        )
        is True
    )
    board.set_piece(
