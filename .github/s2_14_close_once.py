...
SOURCE_SHA = "21406b5e92b6bd42a3a902591dddae22c9b3f16f"
REPORT = Path("docs/RUST_CHESS_ENGINE_V0_2_S2_14_PRODUCTION_VALIDATION_2026-08-06.md")
TRACKER = Path("docs/RUST_CHESS_ENGINE_V0_2_STRENGTH_TODO_2026-08-05.md")
AUDIT = Path("scripts/task_v0_2_strength_audit.sh")
...
REPORT_TEXT = '''# Rust Chess Engine v0.2 — S2-14 Production Candidate Validation
...
S2_14_SECTION = '''# Task S2-14: Production candidate selection and validation — COMPLETE (REJECTED)
...
S2_14_RECORD = '''## S2-14 implementation record
...
start_marker = "# Task S2-14: Production candidate selection and validation — NOT STARTED"
end_marker = "# Task S2-15: Separate activation and v0.2 release gate — NOT STARTED"
if tracker.count(start_marker) != 1 or tracker.count(end_marker) != 1:
    raise SystemExit("unexpected S2-14/S2-15 section markers")
start = tracker.index(start_marker)
end = tracker.index(end_marker)
tracker = tracker[:start] + S2_14_SECTION + tracker[end:]
tracker = replace_once(tracker, "## Program guardrails", S2_14_RECORD + "## Program guardrails", "implementation record")
...
audit = replace_once(
    ...
    "audit checkbox contract",
)
evidence_checks = '''# S2-14 is an evidence-backed rejection, not an activation. Pin the immutable
...
require_literal 'S2-15 activation is not authorized' "$s2_14_report"

'''
audit = replace_once(audit, "# Exact schema and identity boundaries.\n", evidence_checks + "# Exact schema and identity boundaries.\n", "audit evidence insertion")
audit = replace_once(
    audit,
    "  .github/s2_13_payload_03\n)",
    "  .github/s2_13_payload_03\n  .github/s2_14_close_once.py\n  .github/workflows/s2-14-close-once.yml\n)",
    "audit temporary paths",
)
AUDIT.write_text(audit)

workflow = TRACKER_WORKFLOW.read_text()
workflow = replace_once(workflow, "Strength tracker authority through S2-13", "Strength tracker authority through S2-14", "tracker workflow job name")
workflow = replace_once(workflow, "Verify progression and evidence through S2-13", "Verify progression and evidence through S2-14", "tracker workflow step name")
workflow = replace_once(
    workflow,
    "          grep -Fq '| S2-14 | Production candidate selection and validation | **Not started** |' \"$tracker\"",
    "          grep -Fq '| S2-14 | Production candidate selection and validation | **Complete — PVS rejected_strength; v0.1 remains authoritative; inactive** |' \"$tracker\"",
    "tracker workflow summary",
)
workflow = replace_once(
    workflow,
    "          grep -Fq '# Task S2-14: Production candidate selection and validation — NOT STARTED' \"$tracker\"",
    "          grep -Fq '# Task S2-14: Production candidate selection and validation — COMPLETE (REJECTED)' \"$tracker\"",
    "tracker workflow section",
)
workflow = replace_once(
    workflow,
    "          test \"$(grep -Fc -- '- [x]' <<<\"$s2_14\")\" -eq 0\n          test \"$(grep -Fc -- '- [ ]' <<<\"$s2_14\")\" -gt 0",
    "          test \"$(grep -Fc -- '- [x]' <<<\"$s2_14\")\" -eq 31\n          test \"$(grep -Fc -- '- [ ]' <<<\"$s2_14\")\" -eq 0",
    "tracker workflow checkboxes",
)
workflow_report_checks = '''          s2_14_report=docs/RUST_CHESS_ENGINE_V0_2_S2_14_PRODUCTION_VALIDATION_2026-08-06.md
          test -f "$s2_14_report"
          grep -Fq '**Status:** Complete — candidate rejected' "$s2_14_report"
          grep -Fq '**Disposition:** `rejected_strength`' "$s2_14_report"
          grep -Fq '**Activation:** `false`' "$s2_14_report"
          grep -Fq '**Frozen candidate source SHA:** `21406b5e92b6bd42a3a902591dddae22c9b3f16f`' "$s2_14_report"
          grep -Fq 'Report checksum: `bad7aa1f69e9d18e`.' "$s2_14_report"
          grep -Fq 'Report checksum: `d3b883442ec6107b`.' "$s2_14_report"
          grep -Fq 'Artifact `8982304975`' "$s2_14_report"
          grep -Fq 'Artifact `8982375018`' "$s2_14_report"

'''
workflow = replace_once(
    workflow,
    "          integration_report=docs/RUST_CHESS_ENGINE_V0_2_S2_13_INTEGRATION_2026-08-06.md\n",
    workflow_report_checks + "          integration_report=docs/RUST_CHESS_ENGINE_V0_2_S2_13_INTEGRATION_2026-08-06.md\n",
    "tracker workflow evidence",
)
workflow = replace_once(
    workflow,
    "            .github/s2_13_payload_03; do",
    "            .github/s2_13_payload_03 \\\n            .github/s2_14_close_once.py \\\n            .github/workflows/s2-14-close-once.yml; do",
    "tracker workflow temporary paths",
)
Path("/tmp/tracker-close.yml").write_text(workflow)

print("S2-14 closure transformation prepared")
