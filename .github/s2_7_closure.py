...
closeout = '''# Rust Chess Engine v0.2 — S2-7 Principal Variation Search
...
'''
Path("docs/RUST_CHESS_ENGINE_V0_2_S2_7_PVS_2026-08-05.md").write_text(closeout, encoding="utf-8")

tracker_path = Path("docs/RUST_CHESS_ENGINE_V0_2_STRENGTH_TODO_2026-08-05.md")
tracker = tracker_path.read_text(encoding="utf-8")

record = '''## S2-7 implementation record
...
'''
if "## S2-7 implementation record" in tracker:
    raise SystemExit("S2-7 implementation record already exists")
tracker = replace_once(tracker, "# Work order\n", record + "# Work order\n", "work-order insertion")
...
completed_task = '''# Task S2-7: Principal Variation Search candidate — COMPLETE
...
tracker = replace_block(
    tracker,
    r"# Task S2-7: Principal Variation Search candidate — NOT STARTED\n.*?\n# Task S2-8:",
    completed_task,
    "S2-7 task block",
)
tracker = replace_once(
...
Path(".github/s2_7_closure.py").unlink()
